...
export const OutputFormats = {
    ...
    saveTRD(trd: Uint8Array, filename: string, data: Uint8Array, startAddr?: number, fileType?: string) {
    ...
    raw(bytes: Uint8Array | number[], start?: number, length?: number) {
    ...
    tapHeader(name: string, type: number, length: number, param1: number, param2: number) {
    ...
    tapCode(name: string, bytes: Uint8Array, startAddr: number, param2?: number) {
    ...
    tapBasicProgram(name: string, bytes: Uint8Array, autorunLine?: number, lengthWithoutVars?: number) {
    ...
    tapNumbersArray(name: string, bytes: Uint8Array, varLetter?: number) {
    ...
    tapCharsArray(name: string, bytes: Uint8Array, varLetter?: number) {
    ...
    tapHeadless(bytes: Uint8Array, flag?: number) {
    ...
    tapBasicLoader(name: string, codeStart: number, codeLength: number, execAddr?: number) {
    ...
    sna48(memory: Uint8Array | { getRange: (addr: number, len: number) => Uint8Array }, regs: Record<string, any> = {}) {
    ...
    sna128(memory: { getPage?: (page: number) => Uint8Array | null }, regs: Record<string, any> = {}, port7FFD: number = 0) {
    ...
    toBlob(bytes: Uint8Array, mimeType: string = 'application/octet-stream') {
        return new Blob([bytes as any], { type: mimeType });
    },

    // Create download URL
    toDownloadUrl(bytes: Uint8Array, mimeType: string = 'application/octet-stream') {
        const blob = this.toBlob(bytes, mimeType);
        return URL.createObjectURL(blob);
    },

    // Trigger download in browser
    download(bytes: Uint8Array, filename: string, mimeType: string = 'application/octet-stream') {
