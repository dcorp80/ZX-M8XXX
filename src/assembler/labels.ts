...
export const SymbolTable = {
    symbols: {} as Record<string, any>,           // Global symbols: name -> { value, type, defined, used, line, file }
    modules: [] as string[],           // Module stack for MODULE/ENDMODULE
    localPrefix: '',       // Current label prefix for local labels
    tempLabels: {} as Record<number, any[]>,        // Temporary labels: number -> [{ addr, line, defOrder }]
    tempLabelCounters: {} as Record<number, number>, // For generating unique temp label references
    tempDefOrder: 0,       // Global counter for temp label definition order
    tempRefOrder: 0,       // Current reference point in definition order
    prevTempLabels: {} as Record<number, any[]>,
    ...
    // Define a label
    define(name: string, value: number, line: number | null = null, file: string | null = null, type: string = 'label') {
    ...
    reference(name: string, line: number | null = null, file: string | null = null) {
        const fullName = this.getFullName(name);
    ...
    defineTemp(num: number, addr: number, line: number | null = null) {
    ...
    refTempForward(num: number, currentAddr: number, line: number | null = null) {
    ...
    refTempBackward(num: number, currentAddr: number, line: number | null = null) {
        const labels = this.tempLabels[num] || [];
    ...
    parseTemp(name: string, currentAddr: number, line: number) {
    ...
    checkUndefined() {
        const undefinedSyms: { name: string; line: any; file: any }[] = [];
        const syms = this.symbols || {};
        for (const name in syms) {
            const sym = syms[name];
            if (!sym.defined && sym.used) {
                undefinedSyms.push({ name, line: sym.line, file: sym.file });
            }
        }
    ...
    checkUnused() {
        const unused: { name: string; line: any; file: any }[] = [];
        const syms = this.symbols || {};
        for (const name in syms) {
            const sym = syms[name];
    ...
    export() {
        const result: { name: string; value: number; type: string; hex: string }[] = [];
        const syms = this.symbols || {};
        for (const name in syms) {
            const sym = syms[name];
            if (sym && sym.defined) {
    ...
    toObject() {
        const obj: Record<string, { value: number; undefined: boolean }> = {};
        const syms = this.symbols || {};
        for (const name in syms) {
            const sym = syms[name];
            if (sym) {
...
export const EquTable = {
    ...
    define(name: string, value: number, line: number, file: string) {
