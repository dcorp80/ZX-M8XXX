...
export const AsmMemory = {
    ...
    } as Record<string, { pages: number; pageSize: number; slots: { start: number; size: number; page: number }[] }>,

    // Current state
    device: null as { pages: number; pageSize: number; slots: { start: number; size: number; page: number }[] } | null,
    pages: [] as Uint8Array[],           // Array of Uint8Array, one per page
    slots: [] as { start: number; size: number; page: number }[],           // Current slot configuration
    ...
    // Set slot to page
    setSlot(slotNum: number, pageNum: number) {
        if (!this.device) {
    ...
    writeBytes(addr: number, bytes: number[] | Uint8Array) {
    ...
    readByteWithSlots(addr: number, customSlots: { start: number; size: number; page: number }[] | null) {
    ...
    isValidRange(start: number, end: number) {
        if (!this.device) {
    ...
    mmu(slotStart: number, slotEnd: number, pageStart: number) {
        if (!this.device) {
            ErrorCollector.error('No device set');
            return;
        }
