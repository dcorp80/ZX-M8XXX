...
IE.encodeJP = function(ops, addr, syms) {
    if (ops.length === 1) {
...
IE.encodeJR = function(ops, addr, syms) {
...
IE.encodeDJNZ = function(ops, addr, syms) {
    if (ops.length !== 1) {
...
IE.encodeCALL = function(ops, addr, syms) {
    if (ops.length === 1) {
        const val = this.evalExpr(ops[0], syms, addr);
        const [lo, hi] = Z80Asm.wordBytes(val.value);
...
IE.encodeRET = function(ops, addr, syms) {
    if (ops.length === 0) {
...
IE.encodeRST = function(ops, addr, syms) {
    if (ops.length !== 1) {
...
IE.encodePUSHPOP = function(op, ops, addr, syms) {
    if (ops.length !== 1) {
...
IE.encodeEX = function(ops, addr, syms) {
    if (ops.length !== 2) {
...
IE.encodeCB = function(op, ops, addr, syms) {
    if (ops.length !== 1) {
        ErrorCollector.error(`${op} requires 1 operand`);
    }
...
IE.encodeBIT = function(op, ops, addr, syms) {
    if (ops.length !== 2) {
...
IE.encodeIN = function(ops, addr, syms) {
    if (ops.length === 2) {
        const d = ops[0].toUpperCase();
        const s = ops[1].toUpperCase();
...
IE.encodeOUT = function(ops, addr, syms) {
    if (ops.length === 2) {
        const d = ops[0].toUpperCase();
        const s = ops[1].toUpperCase();
...
IE.encodeIM = function(ops, addr, syms) {
    if (ops.length !== 1) {
