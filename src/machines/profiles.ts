...
export const MACHINE_PROFILES: Record<string, any> = {
    '48k': {
        id: '48k',
...
export function getMachineProfile(type: string) {
...
export function getMachineByZ80HwMode(hwMode: number, extHeaderLen: number) {
...
export function getMachineBySzxId(szxId: number) {
    ...
    const fallback: Record<number, string> = { 0: '48k', 6: '128k' };
    return fallback[szxId] || 'unknown';
}

// Helper: check if machine type is 128K-compatible
export function is128kCompat(type: string): boolean {
