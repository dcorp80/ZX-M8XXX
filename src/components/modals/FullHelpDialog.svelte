...
<!-- Preserves same IDs so existing JS event handlers continue to work -->
    <div class="fullhelp-dialog hidden" id="fullHelpDialog">
        <div class="fullhelp-content">
            <div class="fullhelp-header">
                <h3>ZX-M8XXX <span class="help-version">v0.9.37</span> Help</h3>
            ...
            <div class="fullhelp-body">
                <div class="fullhelp-nav">
                    ...
                    <button class="fullhelp-nav-btn" data-section="mapper">Mapper</button>
                    <button class="fullhelp-nav-btn" data-section="pokes">Pokes</button>
                ...
                <div class="fullhelp-section" id="help-overview">
                    ...
                    <ul>
                        ...
                        <li>Game Mapper for capturing rooms and building navigable map grids</li>
                    </ul>
                    <h5>Quick Start</h5>
                    <ol>
                ...
                <div class="fullhelp-section hidden" id="help-labels">
                    ...
                    <ul>
                        ...
                        <li><strong>Profile</strong> (Tools &rarr; Analysis) &mdash; auto-generate labels from runtime behavior</li>
                    </ul>
                    <h5>Display Modes</h5>
                    <ul>
                    ...
                    <h5>Source Filter</h5>
                    <ul>
                        <li><strong>All</strong> &mdash; show all labels (user + profiled + ROM)</li>
                        <li><strong>User</strong> &mdash; manually added labels only</li>
                        <li><strong>Profiled</strong> &mdash; auto-generated by the profiler (marked with <strong style="color:var(--cyan)">P</strong> badge)</li>
                        <li><strong>ROM</strong> &mdash; labels from ROM label files</li>
                    </ul>
                    <p style="font-size: 11px; color: var(--text-dim);">Counts per category shown next to the Clear button.</p>
                    <h5>Features</h5>
                    <ul>
                        <li>Labels appear in operands: <code>CALL main_loop</code></li>
                        <li>Page-aware: 128K paged labels resolve correctly for the active ROM/RAM bank</li>
                        <li>Click label in list to navigate (disasm or memory view)</li>
                        <li>Persistent storage per loaded file (localStorage)</li>
                        <li>Import/Export to JSON files</li>
                    </ul>
                    <h5>Runtime Behavior Profiler</h5>
                    <p>Tools &rarr; Analysis &rarr; <strong>Profile</strong>. Runs the emulator for N frames and classifies subroutines by their behavior:</p>
                    <table><tbody>
                        <tr><th>Label</th><th>Detection</th></tr>
                        <tr><td>isr_handler</td><td>IM 1 interrupt handler at $0038</td></tr>
                        <tr><td>isr_handler_im2</td><td>IM 2 interrupt handler (address from vector table)</td></tr>
                        <tr><td>im2_vector_table</td><td>IM 2 vector table at I&times;256 (marked as DW region)</td></tr>
                        <tr><td>isr_routine</td><td>Called from interrupt context</td></tr>
                        <tr><td>main_loop</td><td>Called in &gt;90% of frames, most callees</td></tr>
                        <tr><td>read_keyboard</td><td>Reads port $xxFE, no screen writes</td></tr>
                        <tr><td>play_beep</td><td>Many OUTs to ULA port ($FE)</td></tr>
                        <tr><td>play_music / ay_write</td><td>OUTs to AY ports ($FFFD/$BFFD)</td></tr>
                        <tr><td>draw_sprite</td><td>Writes screen bitmap only</td></tr>
                        <tr><td>set_attrs</td><td>Writes attributes only</td></tr>
                        <tr><td>draw_screen</td><td>Writes both bitmap and attributes</td></tr>
                        <tr><td>page_memory</td><td>OUTs to 128K paging port</td></tr>
                        <tr><td>init_XXXX</td><td>Called once, in first frame</td></tr>
                        <tr><td>util_XXXX</td><td>Leaf routine called &gt;10 times</td></tr>
                    </tbody></table>
                    <p style="font-size: 11px; color: var(--text-dim);">Profiler preserves user-named labels. Re-running replaces previous profiler labels. ROM subroutines are skipped (except ISR handlers). IM 2 detection: when the CPU uses interrupt mode 2, the profiler identifies the handler address and marks the vector table at I&times;256 as a data (DW) region.</p>
                </div>
                ...
                <div class="fullhelp-section hidden" id="help-breakpoints">
                    ...
                    <h5>Tape &amp; Disk Triggers</h5>
                    <ul>
                        <li><strong>Tape Block</strong>: Break after a tape block is loaded (flash or real-time). Use the skip count to break after N blocks (e.g. skip=4 breaks after the 5th block).</li>
                        <li><strong>Disk Read</strong>: Break after any disk sector read (TR-DOS or +3DOS). Use skip count to break after N reads.</li>
                        <li><strong>Disk Sector</strong>: Break when a specific track:sector is read. Enter as <code>TT:SS</code> (e.g. <code>0:1</code> for track 0, sector 1).</li>
                    </ul>
                    <h5>Port I/O Log</h5>
                ...
                <div class="fullhelp-section hidden" id="help-memory">
                    ...
                    <h5>Memory Dump</h5>
                    <ul>
                        <li>Hex view with inline editing (click byte to edit, Tab for next, Enter to confirm, Escape to cancel)</li>
                        <li>Click and drag to select range</li>
                        <li>Right-click context menu: navigate to disasm/memory, add labels, mark regions, set breakpoints (break on read/write/R/W for single address or selected range)</li>
                    </ul>
                    <h5>Memory Diff</h5>
                    <ol>
                    ...
                    <h5>POKE Search</h5>
                    <p>Find memory locations storing game values (lives, score, etc.) by comparing snapshots:</p>
                    <ol>
                        <li><strong>Snap</strong> &mdash; take a memory snapshot (status shows snap count)</li>
                        <li>Play game until the target value changes (e.g. lose a life)</li>
                        <li><strong>Snap</strong> again &mdash; repeat snap/play for more data points</li>
                        <li><strong>Search</strong> &mdash; scans all RAM, validates every snap-to-snap transition:
                            <ul>
                                <li><code>-1</code> / <code>+1</code> &mdash; value changed by exactly 1 between each snap</li>
                                <li><code>Decreased</code> / <code>Increased</code> &mdash; value went down/up between each snap</li>
                                <li><code>Changed</code> &mdash; value differs between each snap</li>
                                <li><code>Unchanged</code> &mdash; value stayed the same across all snaps</li>
                            </ul>
                        </li>
                        <li>Take more snaps and search again to narrow candidates</li>
                        <li><strong>Filter</strong> &mdash; enter a hex value to keep only candidates where current memory matches. Clear the value and click Filter to undo</li>
                        <li>Click result to view in memory dump; hover to see value at every snap point</li>
                    </ol>
                    <p>Requires at least 2 snapshots. Each search always scans all RAM &mdash; more snaps = fewer candidates. No deduplication: every snap-to-snap pair must match the selected mode.</p>
                    <p><strong>Skip screen</strong> (on by default) &mdash; excludes screen memory (4000-5BFF) to reduce false positives.</p>
                </div>
...
&#125;</pre>
                ...
                <div class="fullhelp-section hidden" id="help-mapper">
                    <h4>Game Mapper</h4>
                    <p>The Mapper tab (Tools &rarr; Mapper) captures game screens and stitches them into a navigable map grid. Useful for documenting game layouts, creating maps, and reverse engineering room structures.</p>
                    <h5>Basic Workflow</h5>
                    <ol>
                        <li>Navigate to a room in the game</li>
                        <li>Click <strong>Capture</strong> (or Ctrl+Space) to capture the screen</li>
                        <li>Use arrow buttons (or Ctrl+Arrow keys) to move to an adjacent grid position</li>
                        <li>Navigate to the next room in the game and capture again</li>
                        <li>Repeat until the map is complete</li>
                    </ol>
                    <h5>Toolbar</h5>
                    <ul>
                        <li><strong>Capture</strong> &mdash; capture the current screen (or configured sub-region) into the current room slot</li>
                        <li><strong>Arrow buttons</strong> &mdash; move the current room position on the grid</li>
                        <li><strong>Room label</strong> &mdash; shows current (X, Y) grid coordinates</li>
                        <li><strong>Floor buttons</strong> (F: &#9660; / &#9650;) &mdash; navigate between floors for multi-level games</li>
                        <li><strong>Blend</strong> &mdash; per-pixel mode blend of all screenshots (picks most frequent color at each pixel, extracting static background and discarding moving sprites). Appears when room has 2+ screenshots</li>
                        <li><strong>Stamp</strong> &mdash; copy a rectangular region from the selected screenshot onto the blended image to correct blend artifacts (appears when blend exists and a screenshot is selected). Opens a dialog over the overview &mdash; drag to select a rectangle, pixels are stamped immediately. Multiple stamps supported; Escape to close</li>
                        <li><strong>Mark color</strong> &mdash; mark the room with a colored diagonal cross (X) overlay using one of the 8 ZX Spectrum colors. Marks appear in the overview, hover popup, and PNG export. Select &ldquo;No mark&rdquo; to remove</li>
                        <li><strong>Delete</strong> &mdash; delete the currently selected screenshot</li>
                        <li><strong>Thumbnails</strong> &mdash; clickable strip of all screenshots for the current room; click to select which is displayed</li>
                        <li><strong>&#9881; (gear)</strong> &mdash; toggle the settings panel</li>
                    </ul>
                    <h5>Settings Panel</h5>
                    <ul>
                        <li><strong>Region</strong> (X, Y, W, H) &mdash; capture sub-region in character cells (default: full screen 0,0,32,24). Locked once any screenshot is captured to prevent dimension mismatches; clear the map to change</li>
                        <li><strong>Highlight</strong> &mdash; color used to outline the current room in the overview</li>
                        <li><strong>Gap H / V</strong> &mdash; horizontal and vertical pixel spacing between rooms in the overview and export (useful for games with non-connected rooms or varying row/column spacing)</li>
                        <li><strong>Zoom</strong> &mdash; overview zoom level: Fit (auto-scale to container), x1 (native pixel size), x2 (double size). x1 and x2 enable scrolling when the map exceeds the container</li>
                        <li><strong>Follow</strong> &mdash; when enabled in x1/x2 zoom mode, the overview auto-scrolls to keep the current room visible</li>
                        <li><strong>Export</strong> &mdash; multi-floor PNG export layout: Separate PNGs (one file per floor), or composite grid (N&times;... for column-count, ...&times;N for row-count). Empty floors are skipped</li>
                        <li><strong>Floor gap</strong> &mdash; pixel spacing between floors in composite PNG exports</li>
                        <li><strong>Game / Level / Author</strong> &mdash; metadata fields saved in the JSON file</li>
                        <li><strong>Save</strong> &mdash; export the map as a JSON file</li>
                        <li><strong>Load</strong> &mdash; import a previously saved JSON map</li>
                        <li><strong>Export PNG</strong> &mdash; export the full stitched map as a PNG image</li>
                        <li><strong>Clear</strong> &mdash; remove all rooms from the map</li>
                    </ul>
                    <h5>Map Overview</h5>
                    <p>The overview canvas shows all rooms on the current floor stitched together. In Fit mode (default), auto-scaled to the available space. In x1/x2 mode, rendered at native/double pixel size with scrollbars; enable Follow to auto-scroll to the current room. The current room is highlighted with a colored border.</p>
                    <ul>
                        <li><strong>Click</strong> a room in the overview to select it</li>
                        <li><strong>Hover</strong> over a room to see a 2x zoom popup preview</li>
                    </ul>
                    <h5>Multi-Floor Support</h5>
                    <p>Games with multiple levels or floors can be mapped using the floor dimension. Each floor is an independent 2D grid. The overview shows only the current floor. Use the floor buttons or Ctrl+PgUp/PgDn to switch floors.</p>
                    <h5>Keyboard Shortcuts</h5>
                    <table><tbody>
                        <tr><th>Key</th><th>Action</th></tr>
                        <tr><td>Ctrl+Space</td><td>Capture screen</td></tr>
                        <tr><td>Ctrl+Arrow keys</td><td>Navigate rooms</td></tr>
                        <tr><td>Ctrl+PgUp / PgDn</td><td>Change floor</td></tr>
                    </tbody></table>
                    <h5>Save/Load Format</h5>
                    <p>Maps are saved as JSON files containing room screenshots (as data URLs), grid coordinates, floor data, capture region, and metadata. Version 2 format supports floors; version 1 files (without floors) are auto-migrated on load.</p>
                </div>
                <div class="fullhelp-section hidden" id="help-pokes">
                    <h4>POKE Manager</h4>
                    <p>The Pokes tab in the debugger panel manages named pokes (multi-address byte patches) and memory value editors. Two-column layout: editors on the left, pokes on the right. Create entries inline or load from JSON files.</p>
                    <h5>Layout</h5>
                    <p>Editors column (left) and pokes column (right) are shown side by side. <strong>Load</strong>, <strong>Save</strong>, and <strong>Clear</strong> buttons are at the top right. Click the <strong>game name</strong> label to edit it.</p>
                    <h5>Adding Pokes</h5>
                    <p>Use the add form at the top of the pokes column: enter a name, address, original value, and poke value, then click <strong>+</strong>. If a poke with the same name already exists, the new patch is added to it (multi-patch building). Addresses and values accept <code>$hex</code>, <code>0xhex</code>, or decimal.</p>
                    <h5>Poke Entries</h5>
                    <p>Each poke has a checkbox to toggle it on/off. Enabling writes the patched bytes to memory; disabling restores the original values. Changes are instant &mdash; no apply button needed. The master checkbox above the poke list enables or disables all pokes at once (indeterminate state when partially enabled). Click <strong>&times;</strong> to remove an entry (restores original bytes if enabled).</p>
                    <h5>Adding Editors</h5>
                    <p>Use the editor add form at the top of the editors column: enter a name, address, and value type (byte or word), then click <strong>+</strong>. <strong>Read</strong> refreshes all editor values from memory.</p>
                    <h5>Memory Editors</h5>
                    <p>Editors provide live read/write access to specific memory addresses. Values accept <code>$hex</code>, <code>0xhex</code>, or decimal input; displayed with <code>$</code> prefix. Editing a value and pressing Enter or leaving the field writes it to memory immediately. Click <strong>&times;</strong> to remove an editor.</p>
                    <p>Word editors use little-endian byte order (low byte at address, high byte at address+1).</p>
                    <h5>JSON File Format</h5>
                    <p>Create poke files in a text editor. Addresses and values accept <code>$hex</code>, <code>0xhex</code>, or decimal formats. Patches use compact <code>[addr, normal, poke]</code> arrays.</p>
                    <pre>
&#123;
    "game": "Game Name",
    "pokes": [
        ...
        &#123;
            ...
            "patches": [["$8E01", "$28", "$18"], ["$8E10", "$3C", "$00"]]
        &#125;
    ],
    "editors": [
        &#123; "name": "Lives", "addr": "$8C2B", "type": "byte" &#125;,
        &#123; "name": "Score", "addr": "$8C30", "type": "word" &#125;
    ]
&#125;</pre>
                    <h5>Fields</h5>
                    <ul>
                        <li><strong>game</strong> &mdash; optional display label shown in the toolbar</li>
                        <li><strong>pokes[].name</strong> &mdash; display name for the poke entry</li>
                        <li><strong>pokes[].enabled</strong> &mdash; initial enabled state (boolean, default false)</li>
                        <li><strong>pokes[].patches[]</strong> &mdash; arrays of <code>[addr, normal, poke]</code></li>
                        <li><strong>editors[].name</strong> &mdash; display label for the editor</li>
                        <li><strong>editors[].addr</strong> &mdash; memory address</li>
                        <li><strong>editors[].type</strong> &mdash; <code>"byte"</code> or <code>"word"</code> (little-endian)</li>
                    </ul>
                    <h5>Project Persistence</h5>
                    <p>Pokes and editors are saved and restored with the project file, including which pokes are currently enabled.</p>
                </div>
